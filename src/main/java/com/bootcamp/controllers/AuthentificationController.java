...
public class AuthentificationController {
    ...
    public ResponseEntity<String> authentification(@RequestBody @Valid LoginInput  loginInput)  {

        HttpStatus httpStatus = null;
        String token,retour;

        token = authentificationService.authentification(loginInput.getUsername(),loginInput.getPassword());

        if (token==null){
            httpStatus = HttpStatus.UNAUTHORIZED;
            retour = "Vérifier vos informations";
        }else {
