...
public class UserController {
    ...
    public ResponseEntity<List<PagUser>> read() throws SQLException, IOException {

        List<PagUser> users = userService.read();
        return new ResponseEntity<List<PagUser>>(users, HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.GET,value = "/{id}")
    @ApiVersions({"1.0"})
    @ApiOperation(value = "Get a user with its id", notes = "Get a user with its id")
    public ResponseEntity<PagUser> read(@PathVariable(name = "id") int id) throws SQLException, IOException {

        PagUser user = userService.read(id);
    ...
    public ResponseEntity<UserRole> setRoleToUser(int idUser,int idRole) throws SQLException, IOException {
        UserRole userRole = userService.setRoleToUser(idUser,idRole);

        return new ResponseEntity(userRole, HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.DELETE,value = "/deleteRole")
    @ApiVersions({"1.0"})
    @ApiOperation(value = "delete a role from a user", notes = "delete a role from a user")
    public ResponseEntity<Boolean> deleteRoleFromUser(int idUser,int idRole) throws SQLException, IOException {
    ...
    public ResponseEntity<List<Role>> getUserRoles(int idUser) throws SQLException, IOException {
        List<Role> roles = userService.getUserRoles(idUser);
